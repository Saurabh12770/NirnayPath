...
function initLogin() {
    ...
    const setUser = (email) => {
        if (email) {
            ...
            if (userSpan) { userSpan.textContent = `Hi, ${email.split('@')[0]}`; userSpan.style.display = 'inline-block'; }
        } else {
            btn.innerHTML = '<i class="fas fa-sign-in-alt"></i> Login';
            if (userSpan) userSpan.style.display = 'none';
        }
    };
    setUser(localStorage.getItem('nirnaypath_user'));

    btn.addEventListener('click', e => {
        e.preventDefault();
        if (localStorage.getItem('nirnaypath_user')) {
            localStorage.removeItem('nirnaypath_user');
            setUser(null);
    ...
    if (doLogin) doLogin.addEventListener('click', () => {
        ...
        if (v) { localStorage.setItem('nirnaypath_user', v); setUser(v); modal.style.display = 'none'; }
...
async function startTest(testName, subject, questionCount = 100, timeLimit = 90) {
    ...
    try {
        ...
        if (!res.ok) throw new Error(`HTTP ${res.status} — ${subject}.json not found`);
...
function showToast(msg, bg = '#1F2937', color = '#FCD34D') {
